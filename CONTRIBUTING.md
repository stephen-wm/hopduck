# Contributing to HopDuck

Thank you for your interest in contributing to HopDuck. This document is the single source of truth for how we develop, review, and ship changes. Read it fully before opening your first pull request — it will save you time and prevent your contribution from being sent back for procedural reasons.
...
17. [Repository Labels](#17-repository-labels)
...
HopDuck is proprietary software. By submitting a contribution, you agree that your contribution becomes the intellectual property of the project owner under the terms of the project's license. Do not contribute code copied from third-party sources without explicit license compatibility checks.
...
```bash
git clone https://github.com/hopDuck-xyz/web.git hopDuck-web
cd hopDuck-web
```
...
| Extension                                               | Purpose               |
| ------------------------------------------------------- | --------------------- |
| OXC (`oxc.oxc-vscode`)                                  | Inline lint errors    |
...
HopDuck follows a **feature-module architecture**. Understanding this is essential before writing any code.
...
│   └── shared/                     Reusable non-feature components: empty states,
...
└── proxy.ts                        Edge middleware: Arcjet security + auth session guard.
```
...
```ASCII
main       — production. Every commit here is deployed to HopDuck.xyz.
...
6. CI runs: typecheck, check (Ultracite), test, build must all pass
...
Should any spelling errors be detected, make sure to fix them before proceeding. Spelling errors triggered by reference of dependency names, e.g. import statements, should be placed in `.cspell/project-terms.txt` file.
...
- **`import type` for type-only imports.** Always use `import type { Foo }` when importing a TypeScript type. This is enforced by Oxlint.
...
- **No inline styles.** Use Tailwind utility classes. If a style requires values outside Tailwind's scale, add it to `src/styles/globals.css` as a custom property, not as an inline `style` prop unless necessary.
- **Accessible by default.** All interactive elements must be keyboard accessible and have appropriate ARIA attributes. Prefer semantic HTML elements (`<button>`, `<nav>`, `<main>`) over `<div>` with event handlers. The Oxlint `jsx-a11y` plugin catches many violations automatically.
...
- Never use `console.log` in application code. Use the structured logger from `src/lib/logger.ts`. This is enforced by Oxlint.
- In edge runtime code (`proxy.ts` only), use the edge-safe logger from `src/lib/edge-logger.ts`.
...
- [ ] All CI checks pass locally: `npm run typecheck && npm run check && npm run test && npm run build`
...
## 17. Repository Labels

HopDuck uses a standardized set of GitHub labels for issue triage, project management, pull requests, and release planning. The label definitions are maintained in [.github/scripts/create-gh-labels.sh](./.github/scripts/create-gh-labels.sh)

To apply the standard HopDuck labels to a repository, ensure GitHub CLI is installed and authenticated:
...
✓ Label "bug" deleted from hopDuck-xyz/web
✓ Label "documentation" deleted from hopDuck-xyz/web
✓ Label "duplicate" deleted from hopDuck-xyz/web
✓ Label "good first issue" deleted from hopDuck-xyz/web
...
```

This wil create or update the labels used by the HopDuck project, including issue type, priority, status, area, etc.

Projects derived from or inspired by HopDuck may use this script to establish the same labeling conventions. Make sure to update the `REPO="hopDuck-xyz/web"` line at the top of the shellscript file to point to your repository.
